...
describe('Images Endpoints', () => {
	...
	describe('PATCH /images/:user_id', () => {
		...
		context(`Given valid patch request`, () => {
			it(`responds 201 and serialized image data`, () => {
				const validPatch = {
				...
				const expectedPatchRes = helpers.makeExpectedAlbum(validPatch);
	...
	describe('DELETE /images/:image_id', () => {
		context(`Given no image found`, () => {
			...
			it(`responds 401 no image found`, () => {
				const invalidImage = {
					id: 123423,
			...
			it(`responds 202 when sucessful`, () => {
				const validImage = testImage[0];
				const testUser = testUsers[0];
				return supertest(app)
